...
int main(int argc, char **argv)
{
    ...
    while (true)
    {
        float distance_cm = HC_SR04_measure();
        // 测量成功才处理
        if (distance_cm > 0.0f)
        {
            if (distance_cm > 20)
...
void HC_SR04_init()
{
    ...
    // 设置输入滤波的分频系数: 先理解 000.STM32/011.TIM/003.Input-Capture/README.md
    TIM_SetClockDivision(TIM1, TIM_CKD_DIV2);

    { // 配置通道: 输入滤波: 过滤掉信号中的毛刺 -> 边沿检测 -> 信号选择 -> 分频 -> 事件(ICPS)

        // 定时器1的配置: 配置为输入捕获
        // 配置通道1
        TIM_ICInitTypeDef icInitConfig = {
            .TIM_Channel = TIM_Channel_1,                // 输入渠道
            .TIM_ICPolarity = TIM_ICPolarity_Rising,     // 边沿检测
            .TIM_ICSelection = TIM_ICSelection_DirectTI, // 信号选择
            .TIM_ICPrescaler = TIM_ICPSC_DIV1,           // 预分频系数
            .TIM_ICFilter = 0x08};                       // 配置输入滤波器，为什么是0x08,查看 000.STM32/011.TIM/003.Input-Capture/README.md
        TIM_ICInit(TIM1, &icInitConfig);

        // 其实还是配置通道1
        TIM_ICInitTypeDef icInitConfig_2 = {
            .TIM_Channel = TIM_Channel_2,                  // 输入渠道
            .TIM_ICPolarity = TIM_ICPolarity_Falling,      // 边沿检测
            .TIM_ICSelection = TIM_ICSelection_IndirectTI, // 信号选择
            .TIM_ICPrescaler = TIM_ICPSC_DIV1,             // 预分频系数
            .TIM_ICFilter = 0x08};                         // 配置输入滤波器，为什么是0x08,查看 000.STM32/011.TIM/003.Input-Capture/README.md
...
float HC_SR04_measure()
{
    ...
    // 手动产生事件,使参数生效: 除了让影子寄存器的值生效外，还可以使CNT的值归零
    TIM_GenerateEvent(TIM1, TIM_EventSource_Update);
    ...
    distance_cm = 340.0f * 100 * pulse_width / 2.0f;
    return distance_cm;
}